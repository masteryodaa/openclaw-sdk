...
!!! note "Session Keys"
    Sessions are identified by a key with the format `agent:{agent_id}:{session_name}`. For example, `agent:my-agent:default`. The SDK builds this automatically — you just provide the agent ID and an optional session name.
